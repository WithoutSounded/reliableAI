...
**DOI**: [10.48550/arxiv.2302.09664](https://doi.org/10.48550/arxiv.2302.09664)
...
arXiv:2302.09664v3 [cs.CL] 15 Apr 2023
                                         ...
                                         diversity, which significantly strengthens the baselines we compare against relative to prior imple-
...
Published as a conference paper at ICLR 2023
     ...
     AUROC                                           AUROC
             0.75                                            0.78
                                                             0.76                Semantic entropy (ours)
             0.70                                                                Normalised entropy
             0.65                                            0.74                Predictive entropy
                                                                                 Lexical similarity
                                                             0.72
                  S
                eneman
...
2   BACKGROUND ON U NCERTAINTY E STIMATION
...
of the conditional P probabilities of new tokens given past tokens, whose resulting log-probability
...
probability is used instead (Malinin & Gales, 2020) becoming an arithmetic mean log-probability
   PN
N     i log p(si | s<i ). Despite empirical success Murray & Chiang (2018), so far this has little
...
3     C HALLENGES IN U NCERTAINTY E STIMATION FOR NLG
...
3.1   S EMANTIC EQUIVALENCE IN LANGUAGE OUTPUTS
...
ification. This is true for question answering, summarisation, artificial assistants. Meanings are
...
the semantic entropy does a better job of capturing the actually low uncertainty.
    ...
    Answer    Likelihood     Semantic likelihood
                               P                       Answer       Likelihood Semantic likelihood
                                                                                 P
      s        p(s | x)          s∈c p(s | x)            s           p(s | x)      s∈c p(s | x)
                                                                             o
...
sequences. That is, the probability ofQthe sequence conditioned on the context comes from condi-
tional token probabilities p(s | x) = i p(si | s<i , x). Instead, we focus on the probability of the
model generating any sequence that shares some meaning. This can be written as
                                      X             XY
                           p(c | x) =    p(s | x) =         p(si | s<i , x).                     (2)
                                       s∈c             s∈c   i

Formally, this treats the output random variable whose event-space is C, a sub-σ-algebra of the
standard event-space S.
3.2    S AMPLING THE EXTREMELY HIGH - DIMENSIONAL LANGUAGE - SPACE
...
In general, any natural language inference classification system (NLI) can be used for our bidirec-
...
requires M 2 -many comparisons in the worst-case. In practice, however, the computational cost is
...
than the main language model (1.5B parameters), each pair comparison is much faster than generat-
ing even one token from the main model. Third, because semantic equivalence is transitive we only
...
Having determined the clusters of generated sequences that mean the same thing, we add their like-
lihoods following Eq. (2) as a way of determining the likelihood of each meaning, rather than each
sequence. We then compute the semantic entropy (SE) as the entropy over the meaning-distribution
                   X                              X  X                 hX           i
       SE(x) = −        p(c | x) log p(c | x) = −            p(s | x) log      p(s | x) .      (3)
                      c                                c    s∈c                 s∈c
...
This is an unbiased estimator of the entropy in Eq. (3). In addition, in some cases we use length-
normalisation as described in Section 3.3.

4.1   H OW THE SEMANTIC ENTROPY ADDRESSES THE CHALLENGES OF NLG
The main inspiration of semantic entropy is to address the semantic invariance of natural language
head-on by converting the problem of uncertainty estimation into meaning-space. In addition, se-
mantic entropy goes some way towards addressing unequal token importance. Generations whose
...
much attention to non-keyword likelihoods. This is one area where supervised language-model-
based uncertainty tools (Lin et al., 2022a; Kadavath et al., 2022) might enable future improvements
that handle this challenge better. We address the challenges of sampling and variable-length gener-
ation using specific details of our sampling procedure in Section 4.
...
predict confidence due to hardware limitations. Our unsupervised method which uses models ‘off-
the-shelf’ avoids these limitations.
Many of the issues that make probabilistic uncertainty estimation in NLG difficult also make au-
tomatic evaluation of NLG difficult. Ott et al. (2018), for instance, study how the performance of
machine translation models suffers because one sentence can be translated in multiple ways. Sim-
ilarly, Sai et al. (2022) discuss how paraphrase detection can be used to evaluate NLG and other
related methods might transfer to uncertainty estimation.
Automatic paraphrase identification can be based on comparing lexical features of two given se-
quences (Fernando & Stevenson, 2008; Issa et al., 2018) or on measuring the similarity between the
...
Transformer-based paraphrase detection models such as EFL (Wang et al., 2021) achieve SotA per-
formance on paraphrase detection benchmarks such as Quora Question Pairs Wang et al. (2017).

Published as a conference paper at ICLR 2023

                                      Semantic entropy                        Predictive entropy
                                      Normalised entropy                      p(True)
                                      Lexical similarity
                                                                       0.80
             0.75
     ...
     AUROC                                                     AUROC   0.70
             0.70
                                                                       0.60
               2.7  B6.7 B    13B                  30B                   2.7  B 6.7B     13B                 30B
                        Number of parameters of model                             Number of parameters of model
                              (a) CoQA                                                 (b) TriviaQA

Figure 2: (a) On CoQA open-book question answering semantic entropy demonstrates better uncer-
...
largest computationally feasible model, with 30B parameters. In all cases we use only a single un-
modified model. There is no ensembling and no stochastic or Bayesian modification. We chose this
...
problem (in which the model answers a question using a supporting paragraph). We use the devel-
opment split (∼8000 questions). We also use TriviaQA (Joshi et al., 2017) as a closed-book QA
...
Baselines. We compare our method against predictive entropy, length-normalised predictive en-
tropy (Malinin & Gales, 2020), p(True) (Kadavath et al., 2022), and lexical similarity (similar to
...
constructing a new natural language question using these possible answers as context before ask-
ing whether the proposed answer is correct and measuring the probability of the completion being
...
6.1   S EMANTIC E NTROPY UNCERTAINTY
...
show that semantic entropy has a significantly higher AUROC than entropy in sequence-probability-
space with and without length-normalisation, as well as the lexical similarity baseline. At the same
time, it performs dramatically better than p(True). Similarly, we find in Fig. 1b that our method out-
performs more for larger model sizes and continues to steadily improve as the model size increases,
...
CoQA contains both longer and shorter ground truth answers. This is why performing length-
normalisation has a large effect for CoQA but no effect for TriviaQA (compare Fig. 2a and Fig. 2b).
TriviaQA is also a more challenging dataset: accuracy of 50.6% against 82.3% for CoQA.
We can better understand the mechanism of action for semantic entropy by examining the differ-
ence between incorrect and correct answers generated by the model. In Table 2 we show that the
...
our predictions, which is that the model is more likely to generate incorrect answers when it is un-
certain about the most likely generation. There are 10 answers generated per question, so there is
...
6.2   H YPERPARAMETERS FOR EFFECTIVE SAMPLING
Adjusting the temperature used for multinomial sampling has two competing effects on the gener-
ated sequences produced by the model. Increasing the temperature increases the diversity of samples
...
Published as a conference paper at ICLR 2023
         ...
         0.72                                                0.775
 AUROC   0.70                                        AUROC
                                                             0.750              Semantic entropy (ours)
         0.68                                                                   Length-normalised entropy
                                                             0.725
         0.66
                              5                10                        0.25      0.50                1.00              1.50
                                                                   0.4
        0.800                                                                                                                   0.9
        0.775
                                                                   0.3                                                          0.8
AUROC   0.750
                       Semantic entropy (ours)
                                                        Accuracy   0.2                                                            Diversity
                                                                                                                                0.7
        0.725          Length-normalised entropy
                                                                                Average correctness of sampled answers          0.6
                       Entropy                                     0.1          Diversity of sampled answers
        0.700                                                                                                                   0.5
                             5                  10                       0.25      0.50              1.00                1.50
                #samples used to estimate entropy                                            Temperature

    (a) (top) CoQA, (bottom) TriviaQA                                                            (b)
...
of possible meanings more fully. Here we measure the diversity      using the average overlap of the
longest sub-sequence among sampled answers (1 − M        −1                          0
                                                             P
                                                      2        s6=s ∈C Rouge-L(s, s )). At the same
...
same thing. Addressing this directly, we introduce semantic entropy—the entropy of the distribu-
tion over meanings rather than sequences—and show that this is more predictive of model accuracy
on QA than strong baselines. Our unsupervised approach using ‘out-of-the-box’ models improves
reproducibility and is easier to deploy. Unsupervised uncertainty may also help address the obser-
vation raised in prior work that supervised uncertainty measures struggle with distribution shift.
For semantic entropy, we introduce a novel bidirectional entailment clustering algorithm which uses
a smaller natural language inference model. Our method therefore represents a middle ground be-
tween fully probabilistic methods and methods that use language models to exploit aspects of natural
...
with relatively clear ground truths. In the future, however, we hope our work on semantic equiva-
lence can pave the way towards progress in settings like summarisation where correctness requires
...
Our aim is to work towards safer AI systems by enabling users to understand the confidence and re-
liability of language model generations. In principle, this could help mitigate many of the potential
...
identifying new considerations and methods for uncertainty quantification in NLG, before deploy-
ment we advise that practitioners conduct extensive evaluations specific to the deployment context
...
Dario Amodei, Chris Olah, Jacob Steinhardt, Paul Christiano, John Schulman, and Dan Mané. Con-
  crete Problems in AI Safety. arXiv, 2016. 1
...
Samuel Fernando and Mark Stevenson. A semantic similarity approach to paraphrase detection. In
  Proceedings of the 11th annual research colloquium of the UK special interest group for compu-
  tational linguistics, pp. 45–52. Citeseer, 2008. 6
...
Taisiya Glushkova, Chrysoula Zerva, Ricardo Rei, and André FT Martins. Uncertainty-aware ma-
  chine translation evaluation. arXiv preprint arXiv:2109.06352, 2021. 6
...
Jordan Hoffmann, Sebastian Borgeaud, Arthur Mensch, Elena Buchatskaya, Trevor Cai, Eliza
  ...
  ing compute-optimal large language models. arXiv preprint arXiv:2203.15556, 2022. 1

Fuad Issa, Marco Damonte, Shay B Cohen, Xiaohui Yan, and Yi Chang. Abstract meaning repre-
  sentation for paraphrase detection. In Proceedings of the 2018 Conference of the North American
  Chapter of the Association for Computational Linguistics: Human Language Technologies, Vol-
  ume 1 (Long Papers), pp. 442–452, 2018. 6
...
Saurav Kadavath, Tom Conerly, Amanda Askell, Tom Henighan, Dawn Drain, Ethan Perez,
  ...
  els (mostly) know what they know. arXiv preprint arXiv:2207.05221, 2022. 1, 2, 3, 6, 7, 8, 9,
...
Yaniv Ovadia, Emily Fertig, Jie Ren, Zachary Nado, David Sculley, Sebastian Nowozin, Joshua
  ...
  systems, 32, 2019. 1
Sebastian Padó, Daniel Cer, Michel Galley, Dan Jurafsky, and Christopher D Manning. Measur-
  ing machine translation quality as semantic equivalence: A metric based on entailment features.
...
Richard Socher, Eric Huang, Jeffrey Pennin, Christopher D Manning, and Andrew Ng. Dynamic
  pooling and unfolding recursive autoencoders for paraphrase detection. Advances in neural infor-
  mation processing systems, 24, 2011. 6
Jeff Speaks. Theories of Meaning. In Edward N. Zalta (ed.), The Stanford Encyclopedia of Philos-
   ophy. Metaphysics Research Lab, Stanford University, 2021. 3
...
A     F URTHER DETAILS ON SEMANTIC ENTROPY
A.1    F URTHER DISCUSSION OF SEMANTIC EQUIVALENCE
We illustrate the distinction between different kinds of equivalence in Table 3. Lexically equivalent
sequences use exactly the same symbols. They are always also semantically and syntactically equiv-
alent (in a given context). Syntactically equivalent sentences have the same grammatical form. But
...
and E(s0 , s00 ) then it follows that E(s, s00 ). However, the implementation of our bidirectional equiv-
alence algorithm permits some classification errors and it is slightly ‘soft’—it will sometimes return
equivalent for pairs that are not quite equivalent. As a result, it is not strictly true that our equiv-
alence relation is transitive, and therefore not strictly true that there is a unique set of equivalence
...
unimportant tokens. Supervised uncertainty methods that make use of language models in the un-
certainty evaluation can potentially take this into account better. In addition, our semantic entropy
approach partly adjusts for this, as discussed in Section 4.1.

A.2    F URTHER ALGORITHMIC DETAILS
...
A.3    I MPACT OF SAMPLING METHOD ON QUALITY OF UNCERTAINTY ESTIMATE
...
identical pattern. Like TriviaQA, the optimal temperature is 0.5 despite a significantly harder prob-
lem with lower accuracy, suggesting that this choice hyperparameter may generalize well. Unlike
...
paper is to show the importance of these choices for uncertainty estimation which has been over-
looked previously, and study the temperature in particular. While we leave the detailed study of
...
Require: context x, set of seqs. {s(2) , . . . , s(M ) }, NLI classifier M, set of meanings C = {{s(1) }}
  for 2 ≤ m ≤ M do
      for c ∈ C do                                             . Compare to already-processed meanings.
          s(c) ← c0                                          . Use first sequence for each semantic-class.
          left ← M(cat(x, s(c) , “<g/>”, x, s(m) ))                   . Does old sequence entail new one?
          right ← M(cat(x, s(m) , “<g/>”, x, s(c) ))                                           . Vice versa?
          if left isSentailment and right is entailment then
              c ← c s(m)                                                           . Put into existing class.
          end if
      end for S
      C ← C {s(m) }                                                . Semantically distinct, gets own class.
  end for
  return C
                      ...
                      AUROC   0.70
                              0.68
                                               Semantic entropy (ours)
                              0.66             Length-normalised entropy
                                        0.25         0.50                    1.00       1.50
                                  0.6                                                          0.8
                                  0.5                                                          0.7
                       ...
                       Accuracy                                                                  Diversity
                                  ...
                                  0.4
                                                                                               0.5
                                  0.3          Average correctness of sampled answers          0.4
...
semantic entropy does clearly outperform the baselines. This finding further underlines the im-
portance of choosing appropriate sampling hyper-parameters when using entropy-based uncertainty
measures in NLG.
...
B        E XPERIMENTAL DETAILS AND ABLATIONS
...
set of answers:
           ...
           • To obtain the answer set for uncertainty estimation, by default we use multinomial sam-
             pling, that is generate() using do sample = True and num beams = 1 . If
...
answer to the given question, we trim all generations by pattern matching for a selection of stop-
words that we observe in the generations: Q:, Question:, QUESTION: and questions:.
...
the assessment of the quality of the uncertainty measure.
           ...
           Rouge-L(y, y 0 ) > 0.5            0.835                 0.810           0.456
           Rouge-1(y, y 0 ) > 0.5            0.835                 0.810           0.457
           Exact matching                    0.828                 0.808           0.394

B.1   R ELIABILITY OF ACCURACY METRIC AS COMPARED TO HUMAN EVALUATION
...
B.2   T ESTING THE BI - DIRECTIONAL ENTAILMENT CLASSIFIER
...
B.3   S ENSITIVITY OF RESULTS TO ACCURACY METRIC
...
matching. For CoQA, getting an exact match is significantly harder.
             ...
             Rouge-L(y, y 0 ) > 0.5       0.7379                   0.7290              0.7657
             Rouge-1(y, y 0 ) > 0.3       0.7672                   0.7533              0.8239
             Rouge-1(y, y 0 ) > 0.5       0.7397                   0.7309              0.7677
      ...
      0.75
                                                        0.4
      0.70            Accuracy on QA task                                Accuracy on QA task
                      AUROC of Semantic entropy                          AUROC of Semantic entropy
                                                        0.2
        1.3
        2.7B           13                  30            1.3
                                                         2.7B            13                     30
...
B.5    E XAMPLE P (T RUE ) FORMAT
The format of the prompt, reproduced here for convenient reference from the original source Kada-
vath et al. (2022), is:
...
Published as a conference paper at ICLR 2023
                                  ...
                                  Semantic entropy                     Predictive entropy
                                  Normalised entropy                   p(True)
                                  Lexical similarity                   Margin probability
                                                                0.80
         0.70
 AUROC                                                  AUROC   0.70
         0.60                                                   0.60
...
B.6       M ARGIN - PROBABILITY BASELINE
...
on CoQA, and the corresponding Hmargin , we see that the margin between two semantically equiv-
alent answers and two semantically distinct answers is often similar. That is, this measure does not
...
We find that if instead of obtaining y(1) and y(2) by multinomial sampling (as in our other ex-
periments) instead of by beam search, this second problem becomes less pronounced and Hmargin
...
semantic uncertainty and thus can not distinguish cases where the model is certain about the cor-
rect answer (but uncertain about the precise formulation) as in row 1, and cases where the model
...
uncertainty in the first case and high uncertainty in the second case.
                          ...
                          y(1)                 y(2)     Hmargin          Semantic entropy
