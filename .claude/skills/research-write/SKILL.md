...
## Bilingual Companion Output

In addition to the English-only LaTeX files (for submission), produce a bilingual markdown companion for each `.tex` file. This serves the same purpose as the bilingual output in earlier pipeline steps: it allows the user (and collaborators who read Traditional Chinese) to review the manuscript's narrative and logic without parsing raw LaTeX.

### `step9_manuscript/01_intro_bilingual.md` and `step9_manuscript/02_relatedwork_bilingual.md`

Structure: interleave English and Traditional Chinese paragraph-by-paragraph. Each paragraph heading indicates the section, followed by the English text, then the Chinese translation.
...
note: "Bilingual companion — NOT for submission. The authoritative version is the .tex file."
---
...
- Keep citation keys in their original form: e.g., "Farquhar et al. (2024)" — do not translate author names
- Keep technical terms in English with Chinese explanation on first mention: e.g., "semantic entropy（語意熵）"
- Keep abbreviations in English: AUROC, ECE, CoT, UQ
- Benchmark names stay in English: TriviaQA, GSM8K, StrategyQA
- Model names stay in English: LLaMA-3-8B, Mistral-7B
- Mathematical notation stays as-is
- Translate the *argument* faithfully — the Chinese version should carry the same persuasive structure as the English, not be a loose summary

The bilingual file is NOT a replacement for the `.tex` file — it is a reading aid. If the user edits the `.tex` file later, the bilingual companion may go stale; note this in the YAML header.

## Communication with User

This skill produces English-only LaTeX output (the manuscript language) plus bilingual markdown companions, and communicates with the user bilingually:
...
- **Markdown companion**: Bilingual (paragraph-by-paragraph interleaving)
